...
class Usb2CanLibUsbBus(BusABC):
    ...
    def __init__(self, channel=None, *args, bitrate=500000, **kwargs):

        self.can = Can8DevUSBDevice(channel)

        # convert to kb/s and cap: max rate is 1000 kb/s
        baudrate = min(int(bitrate // 1000), 1000)

        self.channel_info = "USB2CAN LibUSB device {}".format(
            self.can.get_serial_number()
        )

        connector = "{}; {}".format("USB2Can_LibUSB", baudrate)

        timing = BitTiming(
            tseg1=6, tseg2=1, sjw=1, bitrate=bitrate, f_clock=USB_8DEV_ABP_CLOCK
        )
